...
public:
  ...
  void promote() override {
    if (get_grade() < 5) {
      grade_up();
      add_hourRate();
      cout << "已提升级别为" << get_grade() << ",时薪增加100为"
           << get_hourRate() << endl;
    } else
      cout << "级别已最高" << endl;
  }
  ...
  void save(ofstream &out) override {
    out << "Technician" << ' ' << get_name() << ' ' << get_sex() << ' '
        << get_grade() << ' ' << workHours << ' ' << hourRate << ' '
...
public:
  Manager(string name, string sex, int grade, int fixedSalary, int bm = 0,
          int bd = 0)
      : Employee(name, sex, grade, bm, bd) {
  ...
  void promote() override {
    if (get_grade() < 5) {
      grade_up();
      add_fixedSalary();
      cout << "已提升级别为" << get_grade() << ",固定薪资增加10000为"
           << get_fixedSalary() << endl;
    } else
      cout << "级别已最高" << endl;
  }
  void printInfo() override {
  ...
  void save(ofstream &out) override {
    out << "Manager" << ' ' << get_name() << ' ' << get_sex() << ' '
        << get_grade() << ' ' << fixedSalary << ' ' << get_birthMonth() << ' '
        << get_birthDay() << '\n';
  }
};
...
public:
  Salesman(string name, string sex, int grade, int salesAmount,
           float commissionRate, int bm = 0, int bd = 0)
      : Employee(name, sex, grade, bm, bd) {
  ...
  void promote() override {
    if (get_grade() < 5) {
      grade_up();
      add_commissionRate();
      cout << "已提升级别为" << get_grade() << ",提成比例增加0.1为"
           << get_commissionRate() << endl;
    } else
      cout << "级别已最高" << endl;
  }
  void printInfo() override {
  ...
  void save(ofstream &out) override {
    out << "Salesman" << ' ' << get_name() << ' ' << get_sex() << ' '
        << get_grade() << ' ' << salesAmount << ' ' << commissionRate << ' '
        << get_birthMonth() << ' ' << get_birthDay() << '\n';
  }
};
...
public:
  Salesmanager(string name, string sex, int grade, int fixedSalary,
               int salesAmount, float commissionRate, int bm = 0, int bd = 0)
      : Employee(name, sex, grade, bm, bd),
        Manager(name, sex, grade, fixedSalary, bm, bd),
        Salesman(name, sex, grade, salesAmount, commissionRate, bm, bd) {}
  int get_salary() override {
    return get_fixedSalary() + (int)(get_salesAmount() * get_commissionRate());
  }
  ...
  void promote() override {
    if (get_grade() < 5) {
      ...
      add_fixedSalary();
      cout << "已提升级别为" << get_grade() << ",提成比例增加0.1为"
           << get_commissionRate() << ",固定薪资增加10000为"
           << get_fixedSalary() << endl;
    } else
      cout << "级别已最高" << endl;
  }
  ...
  void save(ofstream &out) override {
    out << "Salesmanager" << ' ' << get_name() << ' ' << get_sex() << ' '
        << get_grade() << ' ' << get_fixedSalary() << ' ' << get_salesAmount()
        << ' ' << get_commissionRate() << ' ' << get_birthMonth() << ' '
        << get_birthDay() << '\n';
  }
};
...
public:
  ...
  void push() {
    ...
    while (1) {
      ...
      if (role == "技术人员" || role == "技术" || role == "jishu") {
        ...
        Employee *p =
            new Technician(name, sex, grade, workHours, hourRate, bm, bd);
        data[size++] = p;
        break;
      }
      // 录入经理信息
      ...
      // 录入销售信息
      else if (role == "销售" || role == "销售员" || role == "销售人员" ||
               role == "xiaoshou" || role == "推销") {
        cout << "请输入姓名：" << endl;
        cout << '>';
        cin >> name;
        ...
        Employee *p =
            new Salesman(name, sex, grade, salesAmount, commissionRate, bm, bd);
        data[size++] = p;
        break;
      }
      // 录入销售经理信息
        ...
        Employee *p = new Salesmanager(name, sex, grade, fixedSalary,
                                       salesAmount, commissionRate, bm, bd);
        data[size++] = p;
        break;
      } else
  ...
  void salary_count_display() {
    ...
    cout << "所有员工的总工资为：" << salary_amount << "\n"
         ...
         << "\n"
         << "所有销售经理的总工资占比为："
         << salary_Salesmanager / salary_amount << endl;
  }
  void promote() {
    string s;
    cout << "请输入员工姓名" << endl;
  ...
  void load_from_file() {
    ...
    while (in >> role) {
      ...
      if (role == "Technician") {
        ...
        float commissionRate;
        in >> name >> sex >> grade >> fixedSalary >> salesAmount >>
            commissionRate >> bm >> bd;
        Employee *p = new Salesmanager(name, sex, grade, fixedSalary,
                                       salesAmount, commissionRate, bm, bd);
        data[size++] = p;
      } else {
